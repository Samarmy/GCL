...
class ImageFolder(data.Dataset):
  ...
  """
    ...
    def __repr__(self):
        ...
        return fmt_str_img.shape[1]

    def focus(self, img, mask):
        ...
        if ((ymax-ymin) > (xmax-xmin)):
            scale_factor = (256/((ymax-ymin)+1))
        else:
            scale_factor = (256/((xmax-xmin)+1))
...
generator_256 = ImageNet_class_conditional_generator(image_size=256, batch_size=loader.batch_size)
...
def aug0(imgs):
    mask_empty = np.zeros((imgs.shape[0], 16, 16))
    return imgs, mask_empty, "_aug0.JPEG"

def aug1(imgs):
    unit = 32
    mask_outer = np.ones((imgs.shape[0], 16, 16))
    mask_outer[:, 4:-4, 4:-4] = 0
    reverse_crop_center = torch.zeros_like(imgs)
    reverse_crop_center[:, :, ((imgs.shape[2]//4)-unit):-((imgs.shape[2]//4)-unit), ((imgs.shape[2]//4)-unit):-((imgs.shape[2]//4)-unit)] = torch.nn.functional.interpolate(imgs, size=(((imgs.shape[3]//2)+unit+unit), ((imgs.shape[3]//2)+unit+unit)), mode="bilinear", align_corners=True)
    return reverse_crop_center, mask_outer, "_aug1.JPEG"


def aug2(imgs):
    unit = 32
    mask_upper_left = np.ones((imgs.shape[0], 16, 16))
    mask_upper_left[:, :8, :8] = 0
    ...
    return reverse_crop_upper_left, mask_upper_left, "_aug2.JPEG"

def aug3(imgs):
    unit = 32
    mask_top = np.ones((imgs.shape[0], 16, 16))
    mask_top[:, :8, 4:-4] = 0
    reverse_crop_center = torch.zeros_like(imgs)
    reverse_crop_center[:, :, :-((imgs.shape[3]//2)-(1*unit)), ((imgs.shape[3]//4)-unit):-((imgs.shape[3]//4)-unit)] = torch.nn.functional.interpolate(imgs, size=(((imgs.shape[3]//2)+(2*unit)), ((imgs.shape[3]//2)+(2*unit))), mode="bilinear", align_corners=True)[:, :, unit:, :]
    return reverse_crop_center, mask_top, "_aug3.JPEG"

def aug4(imgs):
    unit = 32
    mask_upper_right = np.ones((imgs.shape[0], 16, 16))
    mask_upper_right[:, :8, 8:] = 0
    reverse_crop_upper_right = torch.zeros_like(imgs)
    reverse_crop_upper_right[:, :, :-((imgs.shape[2]//2)-unit), ((imgs.shape[2]//2)-unit):] = torch.nn.functional.interpolate(imgs, size=(((imgs.shape[3]//2)+unit), ((imgs.shape[3]//2)+unit)), mode="bilinear", align_corners=True)
    return reverse_crop_upper_right, mask_upper_right, "_aug4.JPEG"
...
def aug5(imgs):
    ...
    mask_right[:, 4:-4, 8:] = 0
    reverse_crop_center = torch.zeros_like(imgs)
    reverse_crop_center[:, :, ((imgs.shape[3]//4)-unit):-((imgs.shape[3]//4)-unit), ((imgs.shape[3]//2)-unit):] = torch.nn.functional.interpolate(imgs, size=(((imgs.shape[3]//2)+(2*unit)), ((imgs.shape[3]//2)+(2*unit))), mode="bilinear", align_corners=True)[:, :, :, :-unit]
    return reverse_crop_center, mask_right, "_aug5.JPEG"

def aug6(imgs):
    unit = 32
    mask_lower_right = np.ones((imgs.shape[0], 16, 16))
    mask_lower_right[:, 8:, 8:] = 0
    reverse_crop_lower_right = torch.zeros_like(imgs)
    reverse_crop_lower_right[:, :, ((imgs.shape[2]//2)-unit):, ((imgs.shape[2]//2)-unit):] = torch.nn.functional.interpolate(imgs, size=(((imgs.shape[3]//2)+unit),((imgs.shape[3]//2)+unit)), mode="bilinear", align_corners=True)
    return reverse_crop_lower_right, mask_lower_right, "_aug6.JPEG"

def aug7(imgs):
    unit = 32
    mask_bottom = np.ones((imgs.shape[0], 16, 16))
    mask_bottom[:, 8:, 4:-4] = 0
    reverse_crop_center = torch.zeros_like(imgs)
    reverse_crop_center[:, :, (((imgs.shape[3]//2)-unit)):, ((imgs.shape[3]//4)-unit):-((imgs.shape[3]//4)-unit)] = torch.nn.functional.interpolate(imgs, size=(((imgs.shape[3]//2)+(2*unit)), ((imgs.shape[3]//2)+(2*unit))), mode="bilinear", align_corners=True)[:, :, :-unit, :]
    return reverse_crop_center, mask_bottom, "_aug7.JPEG"

def aug8(imgs):
    unit = 32
    mask_lower_left = np.ones((imgs.shape[0], 16, 16))
    mask_lower_left[:, 8:, :8] = 0
    reverse_crop_lower_left = torch.zeros_like(imgs)
    reverse_crop_lower_left[:, :, ((imgs.shape[2]//2)-unit):, :-((imgs.shape[2]//2)-unit)] = torch.nn.functional.interpolate(imgs, size=(((imgs.shape[3]//2)+unit), ((imgs.shape[3]//2)+unit)), mode="bilinear", align_corners=True)
    return reverse_crop_lower_left, mask_lower_left, "_aug8.JPEG"
...
def aug9(imgs):
    ...
    reverse_crop_center = torch.zeros_like(imgs)
    reverse_crop_center[:, :, ((imgs.shape[3]//4)-unit):-((imgs.shape[3]//4)-unit), :-((imgs.shape[3]//2)-unit)] = torch.nn.functional.interpolate(imgs, size=(((imgs.shape[3]//2)+(2*unit)), ((imgs.shape[3]//2)+(2*unit))), mode="bilinear", align_corners=True)[:, :, :, unit:]
    return reverse_crop_center, mask_left, "_aug9.JPEG"
...
for ind, data in enumerate(loader):
    ...
    label = data[1].numpy()
    path = data[3]
    for gencrop in gencrop_list:
        imgs, latent_mask, aug_str = gencrop(data[0])
        ...
        print("label_tokens", label_tokens.shape)
        ...
        print("pmap_input_tokens", pmap_input_tokens.shape)
        sample_rngs = jax.random.split(sample_rng, jax.local_device_count())
        print("sample_rngs", sample_rngs.shape)
        ...
        with torch.no_grad():
            ...
            for ind2 in range(gen_images.shape[0]):
                # folder = path[ind2].replace("imagenet", "SwaG").replace(".JPEG", "")
                folder = path[ind2].replace("imagenet", "SwaG").replace(".JPEG", "")
                os.makedirs(folder, exist_ok=True)
                file_name = folder.split("/")[-1]
                original_image = Image.open(path[ind2]).convert("RGB") 
                original_image.save(folder + "/" + file_name + "_0.JPEG")
        
                _gen_img = torchvision.transforms.functional.to_pil_image(torch.clamp(gen_images[ind2], 0, 1))
                _gen_img.save(folder + "/" + file_name + aug_str)
        break
    break
        
    end_time = time.time()
    timey = str((end_time-start_time)*len(loader)/86400.0)
    print(str((ind/len(loader))*100.0) + " " + folder + " " + timey)
    torch.save(((ind/len(loader))*100.0), "GenCrop_Checkpoint.pt")