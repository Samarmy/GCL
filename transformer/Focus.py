...
class ImageFolder(data.Dataset):
  ...
  """
    ...
    def focus(self, img):
        ...
        aug_masks = torch.stack([original_mask, center_mask, upper_left_mask, upper_mask, upper_right_mask, right_mask, lower_right_mask, lower_mask, lower_left_mask, left_mask])
...
generator_256 = ImageNet_class_conditional_generator(image_size=256, batch_size=loader.batch_size*10)
...
with torch.no_grad():
    for ind, data in enumerate(loader):
        ...
        for enum, path in enumerate(paths):
            folder = path.replace("Redo", "Focus").split(".")[0]
            os.makedirs(folder, exist_ok=True)
            file_names = file_names + [(folder + "/" + folder.split("/")[-1] + "_aug" + str(x) + ".JPEG") for x in range(data[0].shape[1])]
            labels = labels + ([data[1][enum],]*data[0].shape[1])
            original_image = Image.open(path).convert("RGB") 
            original_file_name = folder.split("/")[-1]
            original_image.save(folder + "/" + original_file_name + "_R.JPEG")
        
        imgs = data[0].reshape((data[0].shape[0]*data[0].shape[1]), data[0].shape[2], data[0].shape[3], data[0].shape[4])
        masks = data[-1].reshape((data[-1].shape[0]*data[-1].shape[1]), data[-1].shape[3], data[-1].shape[4])
        ...
        masked_tokens = (1-masks) * image_tokens + generator_256.maskgit_cf.transformer.mask_token_id * masks
        masked_tokens = np.reshape(masked_tokens, [imgs.shape[0], -1])
        
        label_tokens = np.expand_dims(labels, axis=-1)
