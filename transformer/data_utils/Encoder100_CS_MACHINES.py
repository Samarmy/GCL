...
net = utils.load_pretrained_feature_extractor(
        '/s/lovelace/e/nobackup/sangmi/sarmst/real_shit/pretrained_models_path/swav_800ep_pretrain.pth.tar',
        "selfsupervised",
        "resnet50",
    )
...
generator = get_model(
        exp_name, "/s/lovelace/e/nobackup/sangmi/sarmst/real_shit/pretrained_models_path", "biggan", device=device
    )
...
files = glob("/s/lovelace/e/nobackup/sangmi/sarmst/real_shit/imagenet/train/*")
...
dataset = utils.get_dataset_images(
        256,
        data_path="/s/lovelace/e/nobackup/sangmi/sarmst/real_shit/imagenet",
        longtail=False,
        split="train",
        test_part=False,
...
train_loader = utils.get_dataloader(
    dataset, 32, shuffle=False, **kwargs
)
...
#             continue
        ...
        for s in range(planet[socket.gethostname()][0], planet[socket.gethostname()][1]):        
...
#                 print(folder + "/" + file_name + "_"  + str(s) + "_G.JPEG")
