        ...
        **layer_kwargs,                 # Arguments for SynthesisLayer.
        ...
        for idx in range(self.num_layers + 1):
            ...
            if to_rgb:
                new_idx = idx + 1
                prev = max(idx - 1, 0)
                is_critically_sampled = True
                use_fp16 = False
                layer = SynthesisLayer(
                    w_dim=self.w_dim, is_torgb=is_torgb, is_critically_sampled=is_critically_sampled, use_fp16=use_fp16,
                    in_channels=int(channels[prev]), out_channels= int(channels[new_idx]),
                    in_size=int(sizes[prev]), out_size=int(sizes[new_idx]),
                    in_sampling_rate=int(sampling_rates[prev]), out_sampling_rate=int(sampling_rates[idx]),
                    in_cutoff=cutoffs[prev], out_cutoff=cutoffs[new_idx],
                    in_half_width=half_widths[prev], out_half_width=half_widths[new_idx],
                    **layer_kwargs)
