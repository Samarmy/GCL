...
def run(config):
    ...
    if config["which_dataset"] == "coco" and test_part:
        all_image_ids = []
    # Loop over loader
    for i, (x, y, image_id, image_path) in enumerate(tqdm(train_loader)):
        if config["which_dataset"] == "coco" and test_part:
        ...
        # Else append to the hdf5
            ...
            if not config["save_features_only"]:
                with h5.File(h5file_name, "a") as f:
                    ...
                    f["paths"].resize(f["paths"].shape[0] + len(image_path), axis=0)
                    f["paths"][-len(image_path) :] = image_path
            if not config["save_images_only"]:
